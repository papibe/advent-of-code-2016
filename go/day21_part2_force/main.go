...
func scramble(data []string, password []rune) string {
	p := []rune(password)
...
func permutations(s []rune, start, size int) [][]rune {
	output := [][]rune{}
	perm := make([]rune, size)

	is_used := make([]bool, size)
	for i := range size {
		is_used[i] = false
	}

	var _permutations func(start, end int)

	_permutations = func(start, end int) {
		if start == end {
			copied := make([]rune, end)
			for i := range end {
				copied[i] = perm[i]
			}
		...
		for i := range size {
			if !is_used[i] {
				perm[start] = s[i]
				is_used[i] = true
				_permutations(start+1, end)
				is_used[i] = false
			}
		}
	}

	_permutations(start, size)
	return output
}

func permutations2(s []rune, start, end int) [][]rune {
	output := [][]rune{}
	perm := make([]rune, end)
	...
	_permutations(start, end)
	return output
}

func solution(filename string, password string) string {
	functions := parse(filename)
	initial := []rune{'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'}
	n := len(initial)

	for _, p := range permutations(initial, 0, n) {
		// fmt.Println(p)
